            ...
            if (node_name == self.start_node):
                for a_node in self.nodes_location:
                    if node_name == a_node[0]:
                        l.append(a_node)
            elif (node_name != self.start_node) and (node_name != self.goal_node):          
                for a_node in self.nodes_location:
                    if node_name == a_node[0]:
                        l.append(a_node)
                        print 'node in highlight path', a_node, l
                        self.draw_node(a_node, 'orange')
                        self.draw_edge(l[i-1], a_node,'orange')
                        i+=1
        for b_node in self.nodes_location:
                    if self.goal_node == b_node[0]:
                        self.draw_edge(b_node, l[-1],'orange')
