...
class BehaviorThreads(object):
    ...
    def __init__(self):
        ...
        # DONEevents queues for communication between processes
    ...
    def spawn_threads(self):
        ...
        #########################################
        # DONEstart a handler thread calling robot_motion_handler(). Passing in self.motion_q
        t_handler = threading.Thread(name='handler thread',target=self.robot_motion_handler, args=(self.motion_q,))
    ...
    # DONEThis function monitors the sensors.

    # DONEBased on sensor readings, it puts events to two event queues. The

    # DONEpossible events are: obstacle, clear(obs free), alert and border.
        ...
        while not self.gQuit:
            ...
            for robot in gRobotList:            
                ...
                if (prox_l > BehaviorThreads.Threshold_obstacle or prox_r > BehaviorThreads.Threshold_obstacle):
                    ...
                    # DONECreate an event and put it in motion queue
                    ...
                    # DONEcreate an event and put it in motion queue. Take out pass
                ...
                if (line_l < BehaviorThreads.Threshold_border or line_r < BehaviorThreads.Threshold_border):
                    ...
                    # DONE create an event and put it in motion queue
    ...
    # DONEImplement your motion handler. You need to get event using the passed-in queue handle and

    # DONEdecide what Hamster should do. Hamster needs to avoid obstacle while escaping. Stop after

    #DONE border is reached.
        ...
        while not self.gQuit:
            ...
            if gRobotList:
                for robot in gRobotList:
                    ...
                    if a.type == "robs":
                        robot.set_wheel(0, -(a.data[0]))
                        robot.set_wheel(1, a.data[1])
                    elif a.type == "lobs":
                        robot.set_wheel(0, a.data[0])
                        robot.set_wheel(1, -(a.data[1]))
                    elif a.type == "clear":
            ...
            if gRobotList:
                for robot in gRobotList:                  
                    if a.type == "alert":
                        b = a.data[0]
                        c = a.data[1]
                        if b > BehaviorThreads.Threshold_obstacle  and c > BehaviorThreads.Threshold_obstacle :
                            nc = (1/c)*2
                            nb = (1/b)*2
                        else:
                            nb = 0
                        ...
                        self.canvas.coords(self.prox_l_id,(165,125-b,165,125))
                        self.canvas.coords(self.prox_r_id,(135,125-c,135,125))      
                       

            # DONEHandles data display and warning(sound).

            # DONE Query event queue(using passed-in queue handle).

            # DONEIf there is an "alert" event, display red beams

            # DONEfor 0.01 seconds and then erase the beams.
